...
def show_home_page():
    ...
    transpose_btn = ttk.Button(main_app_frame,
                               text="二维码整理",
                               command=lambda: show_QR_page())
    transpose_btn.pack(pady=5)
    transpose_btn = ttk.Button(main_app_frame,
                               text="二维码整理（成品二）",
                               command=lambda: show_QR_page_1())
    transpose_btn.pack(pady=5)
...
def show_QR_page():
    ...
    # 选择
    def select_file():
        nonlocal filepath, sheet_name
        filepath = filedialog.askopenfilename(
            title="选择文件",  # 对话框标题
            filetypes=[("表格", "*.xlsx*")]  # 可选择的文件类型
        )
        if filepath:
            path_label.config(text=f"选中的文件：{filepath}")
            execl_file = pd.ExcelFile(filepath)
            options = execl_file.sheet_names
            combobox = ttk.Combobox(main_app_frame, values=options, width=30)
            combobox.grid(row=4, column=0, columnspan=4, padx=10, pady=2)
    ...
    def run_QR():
        nonlocal filepath, sheet_name
        row_max = int(entry_4.get())
        page_max = int(entry_1.get())
        page_column_max = int(entry_2.get())
        page_zoom = int(entry_3.get())
        ...
        for i in range(7, row_max - 1):
            ...
            if pd.isna(df.iloc[i, 4]) or df.iloc[i, 4] == " ":
                ...
                ws.cell(row=new_i + page_max * i_count, column=new_j + 3 * j_count + 1).value = df.iloc[
                    i, 4]
                qr_img = generate_qrcode_image(df.iloc[i, 4], 125)
                ...
                if new_i == page_max + 4:
                    new_i = 4
                    j_count += 1
                if j_count == page_column_max:
                    j_count = 0
                    i_count += 1
        for i in range(page_column_max):
            ws.column_dimensions[get_column_letter(1 + 3 * i)].width = 11.3
            ws.column_dimensions[get_column_letter(2 + 3 * i)].width = 19.89
            ws.column_dimensions[get_column_letter(3 + 3 * i)].width = 16.33
    ...
    text_label_1 = ttk.Label(main_app_frame, text="请填写或选择工作表名称(必填)", font=("Microsoft YaHei UI", 14),
                             anchor="center")
    text_label_1.grid(row=3, column=0, columnspan=4, padx=10, pady=2)

    text_label_2 = ttk.Label(main_app_frame, text="请选择要处理的Excel文件或将文件拖拽到此处",
                             font=("Microsoft YaHei UI", 14, "bold"), anchor="center")
    text_label_2.grid(row=0, column=0, columnspan=4, padx=5, pady=(10, 0))

    # 显示路径
    path_label = tk.Label(main_app_frame, text="未选择文件")
    path_label.grid(row=1, column=0, columnspan=4, padx=5, pady=(10, 0))

    btn.grid(row=2, column=0, columnspan=4, padx=10, pady=(10, 0))

    root.drop_target_register(DND_FILES)
    root.dnd_bind('<<Drop>>', drop_file)

    text_label_3 = ttk.Label(main_app_frame, text="行数:", font=("Microsoft YaHei UI", 13),
                             anchor="center")
    text_label_3.grid(row=5, column=0, columnspan=1, padx=10, pady=2)

    entry_1 = ttk.Entry(main_app_frame, width=13, background="", font=("Microsoft YaHei UI", 13))
    entry_1.grid(row=5, column=1, columnspan=1, padx=10, pady=2)
    entry_1.insert(0, "10")

    text_label_4 = ttk.Label(main_app_frame, text="列数:", font=("Microsoft YaHei UI", 13),
                             anchor="center")
    text_label_4.grid(row=5, column=2, columnspan=1, padx=0, pady=2)

    entry_2 = ttk.Entry(main_app_frame, width=13, background="", font=("Microsoft YaHei UI", 13))
    entry_2.grid(row=5, column=3, columnspan=1, padx=10, pady=2)
    entry_2.insert(0, "3")

    text_label_5 = ttk.Label(main_app_frame, text="缩放:", font=("Microsoft YaHei UI", 13),
                             anchor="center")
    text_label_5.grid(row=6, column=0, columnspan=1, padx=10, pady=2)

    entry_3 = ttk.Entry(main_app_frame, width=13, background="", font=("Microsoft YaHei UI", 13))
    entry_3.grid(row=6, column=1, columnspan=1, padx=10, pady=2)
    entry_3.insert(0, "72")

    text_label_6 = ttk.Label(main_app_frame, text="结尾行:", font=("Microsoft YaHei UI", 13),
                             anchor="center")
    text_label_6.grid(row=6, column=2, columnspan=1, padx=0, pady=2)

    entry_4 = ttk.Entry(main_app_frame, width=13, background="", font=("Microsoft YaHei UI", 13))
    entry_4.grid(row=6, column=3, columnspan=1, padx=10, pady=2)

    btn_1 = ttk.Button(main_app_frame, text="确定", command=run_QR)
    btn_1.grid(row=7, column=0, columnspan=4, padx=0, pady=10)

    btn_3 = ttk.Button(main_app_frame, text='返回首页',
                       command=lambda: show_home_page())
    btn_3.grid(row=7, column=0, columnspan=1, padx=20, pady=10, sticky=tk.W)
