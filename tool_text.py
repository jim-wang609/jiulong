...
class AppState:
    ...
    def __init__(self):
        ...
        self.options = None
        self.combobox = None
        self.root = None # Tkinter主窗口实例
        self.main_app_frame = None # 主应用程序框架

app_state = AppState()


# """将表格列字母（如'A'、'AB'）"""
# Helper function to convert column letters to numbers (A=1, B=2, ..., AA=27, etc.)
def column_letter_to_number(column_letter):
    """
    Convert Excel-style column letters to 1-based column numbers.
    
    Args:
        column_letter (str): The column letter(s) to convert (e.g. 'A', 'AB', 'ZZ')
        
    Returns:
        int: The corresponding 1-based column number
    ...
    for char in reversed(column_letter):
        # Convert character to its position in the alphabet (A=1, B=2, etc.)
        # and multiply by 26^power to handle multi-letter columns
        result += (ord(char.upper()) - ord('A') + 1) * (26 ** power)
        power += 1
    return result
...
def select_file():
    ...
    if file_path_1:
        ...
        combobox = ttk.Combobox(main_app_frame, values=options, width=25)
...
def unmerged(app_state, ws):
    # Iterate through all merged cell ranges in the worksheet
    for merged_cell_range in list(ws.merged_cells):
        # Get the coordinates of the merged cell range
        min_col, min_row, max_col, max_row = merged_cell_range.bounds

        # Unmerge the cells
        ws.unmerge_cells(str(merged_cell_range))

        # Get the value of the top-left cell in the original merged range
        top_left_value = ws.cell(row=min_row, column=min_col).value

        # Fill all cells in the unmerged range with the top-left value
