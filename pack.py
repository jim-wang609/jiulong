...
for i in range(len(arr)):
    qty += arr[i][5]
    weight += arr[i][7]
    if qty > number or weight > 2500:
        if arr[i][5] > number:
            def moreno(arr, i_no, qty_no, df_no, insert_count_no, number):
                density = arr[i_no][7] / arr[i_no][6]
                ...
                df_no.iloc[in_column, 5] = number - (qty_no - arr[i_no][5])
                ...
                insert_count_no += 2
                qty_no, weight_no = arr[i_no][5], df_no.iloc[in_column + 1, 7]
                return qty_no, weight_no, insert_count_no, df_no


            while arr[i][5] > number:
                qty, weight, insert_count, df = moreno(arr, i, qty, df, insert_count, number)
        ...
        if split_string(arr[i][1],split_no) != split_string(arr[i - 1][1],split_no):
            ...
            qty, weight = arr[i][5], arr[i][7]
arr = np.array(df)
qty, pre_empty, set_beyond = 0, 0, 5
for i in range(len(arr)):
    if not arr[i][1] is None:
        qty += arr[i][5]
    else:
        if qty < set_beyond:
            if pre_empty < set_beyond:
